...
function inpcolor(e) {
  //Function that changes the selected color.
  selectedcolor = e.target.value;
}

function submitfun() {
  //function that gets called when you press the submit button to change the canvas size.
...
function gridbuttonfun(e) {
  //function that turns the pixels grid on and off.s
...
function createpng() {
  //function that creates a png image of the canvas.
  ...
  document.write(
    '<a href="data:image/png;base64,' +
      p.getBase64() +
      '" download="download"> click to download </a>'
  );
}

function fillbackground() {
  //function that changes every pixel in the canvas into the selected color.
  const pixels = document.querySelectorAll(".pixel");
  pixels.forEach((pixel) => {
...
function removegridelements(e) {
  //function that removes all the pixels of the canvas.
  while (e.firstChild) {
...
function changeColor(e) {
  //function that changes the color of a pixel.
...
function CreateGrid() {
  //function that creates the pixel in the canvas.
